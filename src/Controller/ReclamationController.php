...
class ReclamationController extends AbstractController
{
    ...
    public function index(EntityManagerInterface $entityManager, ReclamationRepository $repo, Request $request, PaginatorInterface $paginator): Response
    {
        ...
        $idUser = $this->getUser(); // get the id of the logged-in user
    
        $total = $repo->TotalReclamation($filters, $idUser);
        $query = $repo->filterwithdiv($filters, $idUser);
    ...
    public function edit(Request $request, Reclamation $reclamation, EntityManagerInterface $entityManager, FlashyNotifier $flashy): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $entityManager->flush();
            $flashy->success('Reclamation modifier successfully!');

            return $this->redirectToRoute('app_reclamation_index', [], Response::HTTP_SEE_OTHER);
        } else {
            $flashy->error('Reclamation dosent modifier !');
        }
    ...
    public function delete(Request $request, Reclamation $reclamation, EntityManagerInterface $entityManager, FlashyNotifier $flashy): Response
    {
        if ($this->isCsrfTokenValid('delete' . $reclamation->getIdReclamation(), $request->request->get('_token'))) {
            ...
            $entityManager->flush();
            $flashy->success('Reclamation supprimer successfully!');
        }
